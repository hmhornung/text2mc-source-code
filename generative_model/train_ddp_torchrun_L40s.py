...
from text2mc_train_utils import get_dataset_dicts
...
block2tok, block2embedding = get_dataset_dicts(paths["tok2block_file_path"], paths["block2embedding_file_path"])
...
def load_train_objs():
    ...
    np.random.seed(seed)
    random.seed(seed)
    hdf5_filepaths = glob.glob(os.path.join(paths["builds_folder_path"], '*.h5'))
